...
public class AiChatController {
    ...
    @Autowired
    public AiChatController(VectorStore vectorStore, OpenAiChatModel openAiChatModel, ApplicationContext applicationContext, ApplicationContext applicationContext1, GetCurrentUserInfo getCurrentUserInfo) {
    ...
    public String chat(@RequestBody AiMessageWrapper input, @RequestHeader("Authorization") String token) {
        ...
        if (input.getParams().getEnableAgent()) {
            // collect all beans with agent annotation
    ...
    public String chatStream(@RequestParam String prompt, @RequestParam String sessionId) {
        return ChatClient.create(openAiChatModel).prompt()
                .user(prompt)
                .advisors(advisorSpec -> {useChatHistory(advisorSpec, sessionId);})
                .call()
                .content();
    }
